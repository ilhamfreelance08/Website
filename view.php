...
if(isset($_GET['idp'])){
    ...
    $ambilnamapelanggan = mysqli_query($conn, "
SELECT p.idpesanan, pl.namapelanggan
FROM pesanan p
JOIN pelanggan pl ON p.idpelanggan = pl.idpelanggan
WHERE p.idpesanan='$idp'
");

$data = mysqli_fetch_array($ambilnamapelanggan);
...
<tbody>
<?php
$ambilsemuadatapesanan = mysqli_query($conn, 
"SELECT * FROM detailpesanan dp 
...
<tr>
    ...
    <td>
        <a href="hapusproduk.php?idp=<?= $idp; ?>&iddetail=<?= $iddetail; ?>" 
...
<?= $produk['namaproduk']; ?> - <?= $produk['deskripsi']; ?> 
(Stok: <?= $produk['stok']; ?>)
</option>
<?php } ?>
    ...
    <input type="number" name="qty" class="form-control mt-3" placeholder="Jumlah">
